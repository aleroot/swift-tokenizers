...
struct ChatTemplateTests {
    ...
    static let phiTokenizerTask = Task {
        try await HubFixtures.tokenizer(for: "microsoft/Phi-3-mini-128k-instruct")
    }
    ...
    static let tokenizerWithTemplateArrayTask = Task {
        try await HubFixtures.tokenizer(for: "mlx-community/Mistral-7B-Instruct-v0.3-4bit")
    }
    ...
    func templateFromArgumentWithEnum() async throws {
        ...
        // Ground truth from `transformers` (see templateFromConfig): the section after `<s>`
        // starts with a space, which the Prepend normalizer turns into a standalone "▁" (29871).
        let encodedTarget = [
            1, 29871, 518, 25580, 29962, 20355, 915, 278, 14156, 8720, 4086, 29889, 518, 29914, 25580,
            29962,
        ]
        let decoded = tokenizer.decode(tokens: encoded)
        let decodedTarget = "<s>  [INST] Describe the Swift programming language. [/INST]"
    ...
    func jinjaOnlyTemplate() async throws {
        ...
        let tokenizer = try await HubFixtures.tokenizer(for: "FL33TW00D-HF/jinja-test")
        let encoded = try tokenizer.applyChatTemplate(messages: messages)
    ...
    func qwen2_5WithTools() async throws {
        let tokenizer = try await HubFixtures.tokenizer(for: "mlx-community/Qwen2.5-7B-Instruct-4bit")
        #expect(tokenizer.hasChatTemplate)
        ...
        let getCurrentWeatherToolSpec: [String: any Sendable] = [
            "type": "function",
            "function": [
                ...
                "parameters": [
                    ...
                    "properties": [
                        ...
                        "unit": [
                            "type": "string",
                            "enum": ["celsius", "fahrenheit"] as [String],
                        ] as [String: any Sendable],
                    ] as [String: any Sendable],
    ...
    func chatTemplateFromChatTemplateJson() async throws {
        ...
        let qwen2VLTokenizer = try await HubFixtures.tokenizer(for: "mlx-community/Qwen2-VL-7B-Instruct-4bit")
        // Qwen 2.5 VL has a chat_template.json file with a different chat template than the one in tokenizer_config.json.
        let qwen2_5VLTokenizer = try await HubFixtures.tokenizer(for: "mlx-community/Qwen2.5-VL-7B-Instruct-4bit")
    ...
    func applyTemplateError() async throws {
        let tokenizer = try await HubFixtures.tokenizer(for: "google-bert/bert-base-chinese")
        #expect(!tokenizer.hasChatTemplate)
