...
private func makeTokenizer(hubModelName: String) async throws -> PreTrainedTokenizer {
    let config = try await HubFixtures.configuration(for: hubModelName)
    guard let tokenizerConfig = config.tokenizerConfig else {
        throw TestError.tokenizerConfigNotFound
    }
    let tokenizer = try AutoTokenizer.from(tokenizerConfig: tokenizerConfig, tokenizerData: config.tokenizerData)
...
struct TokenizerTests {
    ...
    func gemmaUnicode() async throws {
        let tokenizerOpt = try await HubFixtures.tokenizer(for: "pcuenq/gemma-tokenizer") as? PreTrainedTokenizer
    ...
    func phi4() async throws {
        let tokenizerOpt = try await HubFixtures.tokenizer(for: "microsoft/phi-4") as? PreTrainedTokenizer
        #expect(tokenizerOpt != nil)
        let tokenizer = tokenizerOpt!
    ...
    func tokenizerFromLocalFolder() async throws {
        ...
        let configuration = try LocalModelConfiguration(modelFolder: tokenizerConfigURL.deletingLastPathComponent())

        let tokenizerConfigOpt = configuration.tokenizerConfig
        #expect(tokenizerConfigOpt != nil)
        let tokenizerConfig = tokenizerConfigOpt!
        let tokenizerData = configuration.tokenizerData
    ...
    func legacyLlamaBehaviour() async throws {
        let tokenizerOpt =
            try await HubFixtures.tokenizer(for: "mlx-community/Phi-3-mini-4k-instruct-4bit-no-q-embed")
    ...
    func llama7bCombiningMarks() async throws {
        let tokenizerOpt = try await HubFixtures.tokenizer(for: "huggyllama/llama-7b") as? PreTrainedTokenizer
        #expect(tokenizerOpt != nil)
        let tokenizer = tokenizerOpt!
    ...
    func robertaXLMTokenizer() async throws {
        let tokenizerOpt =
            try await HubFixtures.tokenizer(for: "intfloat/multilingual-e5-small") as? PreTrainedTokenizer
        #expect(tokenizerOpt != nil)
        let tokenizer = tokenizerOpt!
    ...
    func kredorPunctuateAllTokenizer() async throws {
        let tokenizerOpt = try await HubFixtures.tokenizer(for: "kredor/punctuate-all") as? PreTrainedTokenizer
        #expect(tokenizerOpt != nil)
        let tokenizer = tokenizerOpt!
    ...
    func robertaXLMCanonicalTokenizer() async throws {
        let tokenizerOpt = try await HubFixtures.tokenizer(for: "FacebookAI/xlm-roberta-base") as? PreTrainedTokenizer
        #expect(tokenizerOpt != nil)
        let tokenizer = tokenizerOpt!
    ...
    func nllbTokenizer() async throws {
        ...
        for strict in [true, false] {
            let tokenizer = try await HubFixtures.tokenizer(for: "Xenova/nllb-200-distilled-600M", strict: strict)
            #expect(tokenizer is PreTrainedTokenizer)
            #expect(tokenizer.encode(text: "Why did the chicken cross the road?") == expected)
        }
    }
    ...
    func llamaPostProcessor() async throws {
        let tokenizerOpt =
            try await HubFixtures.tokenizer(for: "coreml-projects/Llama-2-7b-chat-coreml") as? PreTrainedTokenizer
        #expect(tokenizerOpt != nil)
        let tokenizer = tokenizerOpt!
    ...
    func localTokenizerFromPretrained() async throws {
        ...
        let source = try await HubFixtures.modelFolder(for: "pcuenq/gemma-tokenizer")
        try FileManager.default.createDirectory(at: downloadDestination, withIntermediateDirectories: true)
        for file in try FileManager.default.contentsOfDirectory(atPath: source.path) where !file.hasPrefix(".") {
            try FileManager.default.copyItem(
                at: source.appendingPathComponent(file), to: downloadDestination.appendingPathComponent(file))
        }
        let downloadedTo = downloadDestination
    ...
    func bertCased() async throws {
        let tokenizerOpt =
            try await HubFixtures.tokenizer(for: "distilbert/distilbert-base-multilingual-cased")
            as? PreTrainedTokenizer
        #expect(tokenizerOpt != nil)
        let tokenizer = tokenizerOpt!
    ...
    func bertCasedResaved() async throws {
        let tokenizerOpt =
            try await HubFixtures.tokenizer(for: "pcuenq/distilbert-base-multilingual-cased-tokenizer")
            as? PreTrainedTokenizer
        #expect(tokenizerOpt != nil)
        let tokenizer = tokenizerOpt!
    ...
    func bertUncased() async throws {
        let tokenizerOpt = try await HubFixtures.tokenizer(for: "google-bert/bert-base-uncased") as? PreTrainedTokenizer
        #expect(tokenizerOpt != nil)
        let tokenizer = tokenizerOpt!
    ...
    func bertHangul() async throws {
        let tokenizerOpt = try await HubFixtures.tokenizer(for: "BAAI/bge-small-en-v1.5") as? PreTrainedTokenizer
        #expect(tokenizerOpt != nil)
        let tokenizer = tokenizerOpt!
    ...
    func robertaEncodeDecode() async throws {
        let tokenizerOpt = try await HubFixtures.tokenizer(for: "FacebookAI/roberta-base") as? PreTrainedTokenizer
        #expect(tokenizerOpt != nil)
        let tokenizer = tokenizerOpt!
    ...
    func tokenizerBackend() async throws {
        let tokenizerOpt =
            try await HubFixtures.tokenizer(for: "mlx-community/Ministral-3-3B-Instruct-2512-4bit")
            as? PreTrainedTokenizer
        #expect(tokenizerOpt != nil)
        let tokenizer = tokenizerOpt!
